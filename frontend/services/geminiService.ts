...
const mergeWithDSP = (data: any, dspFeatures: AudioFeatures | null, fileName: string, hash?: string): Metadata => {
    ...
    return {
        ...
        track:        data.track || 1,
        duration:     (data.duration && data.duration > 0) ? data.duration : (dspFeatures?.duration || 0),
        ...
        // ── AI confidence (0.0–1.0 from LLM Ensemble) ────────────────────────
        // Backend puts this in job.result directly (extracted from _tech_meta by route)
...
export const generateMetadata = async (
    ...
    onStreamToken?: (token: string, field: string) => void,  // real-time streaming callback
        ...
        try {
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    if (d.type === 'stream_token' && onStreamToken) {
                        // Real-time description token — append to live preview
                        onStreamToken(d.token ?? '', d.field ?? 'trackDescription');
                    } else if (d.message && onProgressUpdate) {
                        onProgressUpdate(d.message);
                    }
                } catch (e) {
            ...
            try {
                while (pollCount < maxPolls) {
                    ...
                    if (job.status === 'completed' && job.result) {
                        ...
                        if (aiData.bpm || aiData.key || aiData.duration) {
                            dspFeatures = {
                                ...dspFeatures,
                                bpm:      aiData.bpm      ?? dspFeatures?.bpm,
                                key:      aiData.key      ?? dspFeatures?.key,
                                mode:     aiData.mode     ?? dspFeatures?.mode,
                                // Prefer backend duration (authoritative from audio_features.meta.duration)
                                duration: (aiData.duration && aiData.duration > 0)
                                              ? aiData.duration
                                              : dspFeatures?.duration,
                            } as AudioFeatures;
                        }
