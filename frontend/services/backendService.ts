...
export const backendService = {
    ...
    async generateCertificate(metadata: Metadata, sha256: string, filename: string): Promise<{ ipfs_hash: string; ipfs_url: string; timestamp: string }> {
        const res = await fetch(getFullUrl('/generate/certificate'), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ metadata, sha256, filename }),
        });
