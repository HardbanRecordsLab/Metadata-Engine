...
const EditableVocalStyle: React.FC<{
    ...
    // Normalize: treat 'none', 'None', '' as absent
    const isAbsent = (v?: string) => !v || ['none', 'instrumental', ''].includes(v.toLowerCase());
    const hasNoVocals =
        isAbsent(safeValue.gender) &&
        isAbsent(safeValue.timbre) &&
        isAbsent(safeValue.delivery) &&
        isAbsent(safeValue.emotionalTone);

    if (hasNoVocals) {
        return (
            <span className="px-2.5 py-1 rounded-md text-xs font-medium border bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 italic">
                Instrumental — no vocals detected
            </span>
        );
    }

    const vocalTags: { label: string; withIcon?: boolean }[] = [
        !isAbsent(safeValue.gender) ? { label: safeValue.gender, withIcon: true } : null,
        !isAbsent(safeValue.timbre) ? { label: safeValue.timbre } : null,
        !isAbsent(safeValue.delivery) ? { label: safeValue.delivery } : null,
        !isAbsent(safeValue.emotionalTone) ? { label: safeValue.emotionalTone } : null,
    ].filter(Boolean) as { label: string; withIcon?: boolean }[];

    return (
        <div className="flex flex-wrap gap-2 text-xs">
            {vocalTags.map((tag, i) => (
                <span key={i} className="px-2 py-1 bg-violet-50 dark:bg-violet-900/20 border border-violet-200 dark:border-violet-800/50 rounded-md text-violet-700 dark:text-violet-300 flex items-center gap-1 font-medium">
                    {tag.withIcon && <User className="w-3 h-3" />} {tag.label}
                </span>
            ))}
        </div>
    );
