...
interface ResultsSectionProps {
    ...
    onOpenPricing: () => void;
    audioFeatures?: any;
    streamingDescription?: string; // real-time streamed trackDescription tokens
}
...
const ResultsSection: React.FC<ResultsSectionProps> = ({ isLoading, error, results, showToast, onUpdateResults, currentAnalysis, uploadedFile, onUpdateFile, onBackToBatch, userTier, onOpenPricing, audioFeatures, streamingDescription }) => {
    ...
    useEffect(() => {
        ...
        // Only sync DSP duration if backend didn't already provide one
        if (editedResults.duration && editedResults.duration > 0) {
            durationSyncedRef.current = true;
            return; // Backend duration already set, don't overwrite
        }
        durationSyncedRef.current = true;
    ...
    return (
        <div className="animate-fade-in space-y-6">
            ...
            {/* AI Confidence Score */}
            {editedResults?.confidence !== undefined && (
                <AnimatedSection delay="80ms">
                    <ConfidenceMeter
                        score={Math.round((editedResults.confidence ?? 0.75) * 100)}
                        issues={editedResults.validation_report?.issues ?? []}
                    />
                </AnimatedSection>
            )}

            {/* Real-time description streaming preview (visible during analysis) */}
            {streamingDescription && isLoading && (
                <AnimatedSection delay="90ms">
                    <div className="p-4 rounded-2xl bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-800/50 text-sm italic text-violet-800 dark:text-violet-300 leading-relaxed">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-violet-500 block mb-2">Live Description Preview</span>
            ...
            <div className="grid grid-cols-1 gap-8">
                <div className="space-y-6">
                    ...
                    <AnimatedSection delay="215ms">
                        <MetadataValidationCard metadata={editedResults!} />
                    </AnimatedSection>
                </div>
