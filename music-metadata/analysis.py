...
def sanitize_metadata(metadata: dict) -> dict:
    ...
    if not isinstance(vs, dict):
        ...
        if not gender or gender in ("none", "instrumental", "n/a", "-"):
            # Truly no vocals — normalize to consistent none values
            cleaned["vocalStyle"] = {"gender": "none", "timbre": "none", "delivery": "none", "emotionalTone": "none"}
        else:
            # Has vocals — preserve all non-empty fields, normalize "none" sub-fields
            def _clean_vocal_field(val: any) -> str:
                s = str(val or "").strip()
                return s if s and s.lower() not in ("none", "n/a", "-") else "none"
            cleaned["vocalStyle"] = {
                "gender":       str(vs.get("gender") or "").strip(),  # Keep original casing for gender
                "timbre":       _clean_vocal_field(vs.get("timbre")),
                "delivery":     _clean_vocal_field(vs.get("delivery")),
                "emotionalTone": _clean_vocal_field(vs.get("emotionalTone")),
            }
...
async def process_analysis(
    ...
    try:
        ...
        # L1: SQLite in-process cache (sub-millisecond, survives within process)
        from app.utils.caching import cache
        cached_result = cache.get(file_hash)

        # L2: PostgreSQL AnalysisHistory (survives restarts & horizontal scaling)
        if not cached_result:
            history_row = (
                db.query(AnalysisHistory)
                .filter(AnalysisHistory.file_hash == file_hash)
                .order_by(AnalysisHistory.created_at.desc())
                .first()
            )
            if history_row:
                stored = history_row.metadata or history_row.result
                if stored and isinstance(stored, dict) and stored.get("mainGenre"):
                    cached_result = stored
                    cache.set(file_hash, cached_result)  # warm L1
                    logger.info(f"Cache L2-HIT (DB) for Job {job_id} hash={file_hash[:16]}...")

        if cached_result:
            logger.info(f"Cache HIT for Job {job_id} — skipping LLM pipeline")
            sanitized = sanitize_metadata(dict(cached_result))
            job.result  = sanitized
            job.status  = "completed"
            job.message = "Analysis complete (from cache — instant result)."
            db.commit()
        ...
        metadata = await analyzer.analyze_fresh_track(
            ...
            job_id=job_id,       # enables real-time WS description streaming
        )
        ...
        # Attach LLM confidence so frontend can display ConfidenceMeter
        # (_tech_meta was already popped above; read confidence from it)
        if tech_meta.get('confidence') is not None:
            final_metadata['confidence'] = float(tech_meta['confidence'])
