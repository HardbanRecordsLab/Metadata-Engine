...
class FreshTrackAnalyzer:
    ...
    async def analyze_fresh_track(
        ...
        time_budget: int = 45,
        job_id: str = None,          # Needed for WebSocket streaming
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            include_lyrics: Czy transkrybować lyrics (+8-10s, +40MB)
            time_budget:    Max czas w sekundach
            job_id:         Job UUID — required for real-time WS streaming
        ...
        try:
            ...
            if remaining < 3:
                ...
                llm_timeout = max(5, remaining - 1)

                # ── Real-time description streaming (concurrent with consensus) ──
                # If job_id is present, start streaming trackDescription tokens
                # to the frontend immediately via WebSocket, so the user sees
                # text appearing ~3s in rather than waiting for full analysis.
                stream_task = None
                ...
                # Collect streamed description (use as high-quality override if better)
                if stream_task:
                    try:
                ...
                # Override consensus description only if streamed version is longer/richer
                ...
                # ── Auto re-analyze on low confidence ─────────────────────────
                # If the consensus confidence is below threshold AND we still
                # have time AND the original was Flash mode, run a Pro pass.
                # This catches niche genres / unusual production styles that
                # 2-LLM Flash voting gets wrong.
                ...
                if (
                    ...
                    and remaining_after_l2 > 12  # need at least 12s for Pro pass
                ):
                    logger.info(
                        f"Confidence {consensus_confidence:.2f} < {CONFIDENCE_THRESHOLD} "
                        f"— running Pro re-analysis pass ({remaining_after_l2:.0f}s remaining)"
                    )
                    ...
                    try:
                        ...
                        if pro_confidence > consensus_confidence:
                            logger.info(
                                f"Pro pass improved confidence: "
                                f"{consensus_confidence:.2f} → {pro_confidence:.2f}"
                            )
                            # Keep the better streamed description if present
                            if streamed_description and len(streamed_description) > len(
                            ...
                            logger.info(
                                f"Pro pass did not improve ({pro_confidence:.2f} ≤ "
                                f"{consensus_confidence:.2f}) — keeping Flash result"
                            )
                    except asyncio.TimeoutError:
