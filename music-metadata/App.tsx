...
const AppContent: React.FC = () => {
    ...
    const [streamingDescription, setStreamingDescription] = useState<string>('');
        ...
        for (let i = 0; i < itemsToProcess.length; i++) {
            ...
            try {
                ...
                */
                ...
                // Reset streaming preview for this new item
                ...
                const responseData = await generateMetadata(
                    ...
                    isFresh,
                    (token, field) => {
                        // Real-time streaming of trackDescription tokens via WebSocket
                        if (field === 'trackDescription') {
                            setStreamingDescription(prev => prev + token);
                        }
                    }
                );

                // Clear streaming preview once analysis is complete
                setStreamingDescription('');
                                ...
                                {view === 'results' && activeAnalysis && (
                                    <div className="max-w-7xl mx-auto">
                                        <ResultsSection
                                            ...
                                            userTier={userTier}
                                            onOpenPricing={() => setIsPricingOpen(true)}
                                            audioFeatures={activeAnalysis.audioFeatures}
