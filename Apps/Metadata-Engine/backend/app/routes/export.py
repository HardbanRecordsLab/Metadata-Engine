...
def metadata_to_csv_row(metadata: Dict[str, Any], filename: str, ipfs_hash: str = None, ipfs_url: str = None) -> Dict[str, str]:
    """
    Convert metadata dictionary to CSV row compatible with MP3Tag and DAWs.
    Fields match the unified schema in analysis.py ALLOWED_METADATA_KEYS.
    """
    vocal_style = metadata.get('vocalStyle', {}) or {}
    energy_level = metadata.get('energyLevel') or metadata.get('energy_level', '')
    ...
    return {
        ...
        'SUBGENRE':        join_list(metadata.get('additionalGenres', [])),
        'MOOD':            join_list(metadata.get('moods', [])),
        'INSTRUMENTATION': join_list(metadata.get('instrumentation', [])),
        'USAGE':           join_list(metadata.get('useCases', [])),
        'KEYWORDS':        join_list(metadata.get('keywords', [])),
        # Description
        ...
        'P_LINE':         metadata.get('pLine', ''),
        'SHA256':         metadata.get('sha256', ''),
        'LICENSE':        metadata.get('license', ''),
        # Extended AI fields
        'ENERGY_LEVEL':        energy_level,
        'MOOD_VIBE':           metadata.get('mood_vibe', ''),
        'MUSICAL_ERA':         metadata.get('musicalEra', ''),
        'PRODUCTION_QUALITY':  metadata.get('productionQuality', ''),
        'DYNAMICS':            metadata.get('dynamics', ''),
        'TARGET_AUDIENCE':     metadata.get('targetAudience', ''),
        'TEMPO_CHARACTER':     metadata.get('tempoCharacter', ''),
        'ANALYSIS_REASONING':  metadata.get('analysisReasoning', ''),
        # DSP metrics
        'DYNAMIC_RANGE_DB':   str(metadata.get('dynamicRange', '')),
        'SPECTRAL_CENTROID_HZ': str(metadata.get('spectralCentroid', '')),
        'ACOUSTIC_SCORE':     str(metadata.get('acousticScore', '')),
        'HAS_VOCALS':         str(metadata.get('hasVocals', '')),
        # IPFS / External
...
async def export_csv(job_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        fieldnames = [
            # Identity
            'filename', 'title', 'artist', 'album', 'albumartist', 'year', 'track',
            # Sonic / Technical
            'MAIN_INSTRUMENT', 'TKEY', 'MODE', 'bpm', 'LANGUAGE',
            # Genre & Classification
            'genre', 'SUBGENRE', 'MOOD', 'INSTRUMENTATION', 'USAGE', 'KEYWORDS',
            # Description
            'comment',
            # Vocal Style
            'VOCAL_STYLE_GENDER', 'VOCAL_STYLE_TIMBRE', 'VOCAL_STYLE_DELIVERY', 'VOCAL_STYLE_EMOTIONALTONE',
            # Credits & Legal
            'copyright', 'publisher', 'composer', 'lyricist', 'producer',
            'CATALOGNUMBER', 'isrc', 'iswc', 'upc', 'P_LINE', 'SHA256', 'LICENSE',
            # Extended AI fields
            'ENERGY_LEVEL', 'MOOD_VIBE', 'MUSICAL_ERA', 'PRODUCTION_QUALITY',
            'DYNAMICS', 'TARGET_AUDIENCE', 'TEMPO_CHARACTER', 'ANALYSIS_REASONING',
            # DSP metrics
            'DYNAMIC_RANGE_DB', 'SPECTRAL_CENTROID_HZ', 'ACOUSTIC_SCORE', 'HAS_VOCALS',
            # External
            'ipfs_hash', 'ipfs_url',
        ]
